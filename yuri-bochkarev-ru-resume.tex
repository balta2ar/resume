...
\noindent
Linux Python backend-разработчик с более чем 10 годами опыта в разработке,
...
\job  % sets the header for the section and includes any subsections
    ...
    {Москва}
    {
        \position  % sets the header for a subsection and contains usually body text
            {Старший разработчик отдела Technology}
            {май 2016 -- настоящее время}
            {
                \begin{itemize}
            ...
            {январь 2013 -- июль 2014}
            {
                \begin{itemize}
                    \item{Поддержка, развитие и рефакторинг архитектуры сервиса
                          доставки данных (Python, \acr{BASH}, Django)}
                \end{itemize}
            }
    }
